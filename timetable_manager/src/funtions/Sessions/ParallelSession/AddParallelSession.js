...
class addparallel extends React.Component {
    ...
    onFormSubmit = async (e) => {
        ...
        if(this.validate()){
            ...
            const result = await parallel.addparallel({session_01 , session_02});
            if(result.status == 200 ){
                CONFIG.setToast(result.message);
                this.props.history.push("/student/ParellalSession")
            }else{
        ...
        <main>
            ...
            <div className="row" >
                ...
                <div className="col-md-12 mt-1 mb-1" >
                    ...
                    <div className="col-md-12 mt-1 mb-1" >
                            <button type="submit" className="btn-outline-info mt-2 btn btn-sm px-2 ">Add Parallel Session</button>
                    </div>
                </div>
                </form>
                </div></div>
    ...
    validate = () => {
        const { session_01 , session_02  ,  parallelList } = this.state
        let error  = 0;
        let error_message = '';

        let s_01 = parallelList.find( item => item._id == session_01);
        let s_02 = parallelList.find( item => item._id == session_02);
        ...
        if( parseInt(s_01.duration) != parseInt(s_02.duration)){
            this.setState({
                error_message : 'Both sessions should be same duration !',
                error : true
            })
            return false
        }
