...
import {getAllSessions , getAllRooms, addRoomsToSession} from '../../controllers/Suitable.Controller'


class RoomsForSession extends React.Component {
    ...
    loadData = (first) => {
      ...
      .then( results => {
         if(first){
             console.log("sessions in load: ", results.result)
          this.setState({
             groups : results.result ,
             selected_lec : results.result.length > 0 ? results.result[0]._id : {} , 
             selected_rooms : results.result.length > 0 ? results.result[0].rooms : [],
             loading : false
         })
      }else{
          this.setState({
    ...
    onFormSubmit = (e) => {
      ...
      addRoomsToSession(data).then( result => {
          if(result.status == 200 ){
              Config.setToast('Rooms for the Session Added Successfully!');
              this.loadData(false);
          }else{
      ...
      formValueChange = (e) => {
          ...
          let groups = this.state.groups.find( item => item._id == e.target.value )
                       ...
                       <div className="col-12 shadow-sm rounded bg-white mt-3" >
                          ...
                          <tbody>
                              { !loading && groups.map( (item,i) =>  (
                                  <tr>
                                      <td>{("0" + (i+1)).slice(-2)}</td>
                                      <td>{item.subject.name + " - " + item.tag.name + " - " + item.lecturer.name}</td>
   ...
   renderGroups = () => {
       console.log("these are groups ", this.state);
       return this.state.groups.map( i => {
           return {
               label : i.subject.name + " - " + i.tag.name + " - " + i.lecturer.name + " - " + i.group.subgroup_ID,
               value : i._id
           }
       })
   }
...
export default RoomsForSession;