...
import {getAllTags , getAllRooms, addRoomsToTags} from '../../controllers/Suitable.Controller'

class RoomsforTag extends React.Component {
  ...
  loadData = (first) => {
    ...
    .then( results => {
       if(first){
        this.setState({
           tags : results ,
           selected_tag : results.length > 0 ? results[0].name : {} , 
           selected_rooms : results.length > 0 ? results[0].rooms : [],
           loading : false
       })
    }else{
        this.setState({
  ...
  onFormSubmit = (e) => {
    ...
    const data = {
        name: this.state.selected_tag,
        rooms: this.state.selected_rooms
    }


    addRoomsToTags(data).then( result => {
        if(result.status == 200 ){
            Config.setToast('Rooms for the tag Added Successfully!');
            this.loadData(false);
        }else{
    ...
    formValueChange = (e) => {
        ...
        let tag = this.state.tags.find( item => item.name == e.target.value )
        this.setState({
            selected_tag : e.target.value ,
            selected_rooms : tag.rooms,
            loading : false 
        });
    }
