...
import {getAllConSessions , getAllRooms, addRoomsToConSession} from '../../controllers/Suitable.Controller'

class ConsecutiveSession extends React.Component {
    ...
    loadData = (first) => {
      ...
      .then( results => {
         if(first){
          ...
          this.setState({
             groups : results.data ,
             selected_lec : results.data.length > 0 ? results.data[0]._id : {} , 
    ...
    onFormSubmit = (e) => {
      ...
      console.log("Groups object id: ", this.state.groups);
      let groups = this.state.groups.find( item => item._id == this.state.selected_lec )
      console.log("from array: ", groups);
      ...
      addRoomsToConSession(data).then( result => {
          if(result.status == 200 ){
              Config.setToast('Room for the Consecutive Session Added Successfully!');
              this.loadData(false);
          }else{
      ...
      dropdownOnChange = (e) => {
          ...
          if( selected_rooms.indexOf(e.target.value) == -1 && e.target.value != '')
              this.setState({ selected_rooms : [e.target.value], dropdown : ''})
      }
                      ...
                      <form onSubmit={(e) => this.onFormSubmit(e)}>
                       ...
                       <div className="col-12 shadow-sm rounded bg-white mt-3" >
                          ...
                          <thead>
                              ...
                              <th className="font-08 text-dark2 ">Session 1</th>
                              <th className="font-08 text-dark2 ">Session2</th>
                              <th className="font-08 text-dark2 ">Selected Room</th>
                              </tr>
                          </thead>
                          <tbody>
                              { !loading && groups.map( (item,i) =>  (
                                  <tr>
                                      <td>{("0" + (i+1)).slice(-2)}</td>
                                      <td>{item.session_01.subject + " " + item.session_01.tag}</td>
                                      <td>{item.session_02.subject + " " + item.session_02.tag}</td>
   ...
   renderGroups = () => {
       ...
       return this.state.groups.map( i => {
           return {
               label : i.session_01.subject + " " + i.session_01.tag + " - " + i.session_02.subject + " " + i.session_02.tag,
               value : i._id
           }
       })
   }
